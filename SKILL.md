---
name: theia
description: Eclipse Theia extension development — InversifyJS DI (symbol+interface services, rebind), contribution points/providers/filters, commands/menus/keybindings, widgets (ReactWidget, TreeWidget, WidgetFactory, view contributions, React 19 shared imports), frontend/backend lifecycle, custom editors, label providers, breadcrumbs, tab bar preview, MessageService, property view, preferences, perspectives, dynamic toolbar, i18n (nls), tasks, telemetry, Theia AI (agents, variables, tools, chat renderers), VS Code extension custom API, frontend/backend JSON-RPC. Use when writing or reviewing code in a repo depending on @theia/*, or files like *-frontend-module.ts / *-backend-module.ts.
---
